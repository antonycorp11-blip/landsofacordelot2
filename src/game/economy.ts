...
export function quoteAt(poi: PointOfInterest, id: GoodId, s: GameState = getState()): MarketQuote {
  ...
  const security = Math.max(-30, Math.min(30, s.regionSecurity[poi.regionId] ?? 0));
  const roadPressure = 1 - security * 0.003;
  const marketPrice = Math.max(2, Math.round(good.basePrice * scarcity * (1.12 - Math.min(0.28, (production - 0.7) * 0.15)) * pulse * prosperity * warPressure(poi, s) * roadPressure));
